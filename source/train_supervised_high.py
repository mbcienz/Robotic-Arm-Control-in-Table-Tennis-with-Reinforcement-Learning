...
# Read the dataset
dataset = pd.read_csv('saved_dataset/dataset_high.csv', header=None)

# Manually assign column names
dataset.columns = ['z', 'j5', 'j7']
...
# Prepare the data
X = dataset[['z']].values  # Use z as the input
y = dataset[['j5', 'j7']].values  # Use j5 and j7 as the output
...
joblib.dump(scaler_X, 'saved_models/supervised/high_scaler_X.pkl')
joblib.dump(scaler_y, 'saved_models/supervised/high_scaler_y.pkl')
...
for epoch in range(epochs):
    ...
    if val_loss.item() < best_val_loss:
        ...
        torch.save(model.state_dict(), 'saved_models/supervised/model_high.pth')  # Save the best model
        patience_counter = 0
    else:
        patience_counter += 1
